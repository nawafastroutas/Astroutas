...
- شاشة افتتاحية (Splash / Loading Screen) فخمة بهوية الموقع (بني/كريمي)
...
> ⚠️ **`index.html` ملف واحد مكتفٍ بذاته بالكامل** — شاشة التحميل (CSS+JS)
> والشعار وكل الأيقونات و PWA manifest كلها **مضمَّنة داخله** (inline/base64)،
> فلا حاجة لرفع أي مجلد آخر معه. ارفع `index.html` لوحده على الاستضافة
> ويشتغل الموقع كامل بشاشة التحميل تمامًا كما هي. (الاستثناء الوحيد:
> `sw.js` — إن رفعته بجانبه يفعّل خاصية "تثبيت التطبيق" الفعلية على
> أندرويد/Chrome؛ بدونه الموقع يعمل طبيعيًا فقط بدون ذلك الخيار.)
...
| الملف | الدور |
|---|---|
| `index.html` | **الموقع الفعلي — ملف واحد مكتفٍ بذاته.** شاشة التحميل والشعار والأيقونات و manifest كلها مضمَّنة بداخله inline، هذا هو الملف الوحيد المطلوب رفعه للاستضافة. |
| `sw.js` | service worker اختياري (لتفعيل "تثبيت التطبيق" الحقيقي فقط) — ارفعه بجانب `index.html` إن أردت هذه الميزة. |
| `src/splash/splash.css` | **مرجع/تطوير فقط** — نفس تنسيق شاشة التحميل المضمَّن في `index.html`، محفوظ هنا لسهولة التعديل مستقبلًا (عدِّله هنا ثم أعد تضمينه). |
| `src/splash/splash.js` | **مرجع/تطوير فقط** — نفس متحكّم الشاشة المضمَّن في `index.html`. |
| `src/splash/react/SplashScreen.jsx` | نسخة React / Next.js (مرجع إن استُخدم إطار عمل لاحقًا) |
| `manifest.webmanifest` | **مرجع فقط** — نفس محتواه مضمَّن كـ data URI داخل `index.html`. |
| `assets/icons/`, `assets/brand/` | **مرجع فقط** — نفس الصور مضمَّنة base64 داخل `index.html`. |
